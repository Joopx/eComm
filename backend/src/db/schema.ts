...
export const users = pgTable("users",{
    id: text("id").primaryKey(),   //here clerk id would be used for userid
...
//using relations we define how tables connect to each other which would enable drizzle's query API  to automatically join related data when using with {relationName : true}
...
//a product can have 1 user and many comments
//one() means a single related record , many mean many ...
...
//fields : foreign key col in products.userId table 
//refernces : primary key col in the related table (users.id )
